...
?>
<p><strong><?php printf( __( 'Besuche die Registerkarte Upgrades, um weitere Foren zu erhalten. Derzeit bist Du auf %s beschränkt', 'psforum' ), $forums_max_forums ); ?></strong></p>
<?php
}
...
add_action('admin_print_styles-toplevel_page_psforum', 'forums_admin_styles');
add_action('admin_print_scripts-toplevel_page_psforum', 'forums_admin_scripts');
...
function forums_plug_init() {
	...
	if ($forums_enable_upgrades == 1) {
		if (function_exists('upgrades_register_feature')){
			...
			upgrades_register_feature( '68daf8bdc8755fe8f4859024b3054fb8', __( 'Foren', 'psforum' ), __( 'Zusätzliche Foren', 'psforum' ) );
	...
	if ( defined( 'WPMU_PLUGIN_DIR' ) && file_exists( WPMU_PLUGIN_DIR . '/' . basename( __FILE__ ) ) ) {
		load_muplugin_textdomain( 'psforum', WPMU_PLUGIN_DIR .'pscp-forum/languages' );
	} 
	else {
		load_plugin_textdomain( 'psforum', false, 'pscp-forum/languages' );
	}
	
	if (is_admin()) {
		wp_register_script('farbtastic', plugins_url('pscp-forum/assets/js/farbtastic.js'), array('jquery'));
		wp_register_script('forums_admin_js', plugins_url('pscp-forum/assets/js/forums-admin.js'), array('jquery','farbtastic'), $forums_current_version, true);
		wp_register_style('forums_admin_css', plugins_url('pscp-forum/assets/css/wp_admin.css'));
	}
}
...
function forums_make_current() {
	...
	if ( !empty($wpdb->base_prefix) ) {
		...
		if (get_site_option( "forums_version" ) == '') {
			add_site_option( 'forums_version', '1.0.0' );
		}
		...
		if (get_option( "forums_version" ) == '') {
			add_option( 'forums_version', '1.0.0' );
		}
...
function forums_plug_pages() {
	...
	if ( $forums_enable_upgrades && FORUM_DEMO_FOR_NON_SUPPORTER && function_exists('is_pro_site') && !is_pro_site()) {
		add_menu_page( __( 'Foren', 'psforum' ), __( 'Foren', 'psforum' ), 'manage_options', 'psforum', 'forums_non_supporter_output', 'dashicons-format-status');
	} else {
		add_menu_page( __( 'Foren', 'psforum' ), __( 'Foren', 'psforum' ), 'manage_options', 'psforum', 'forums_manage_output', 'dashicons-format-status');
	}
}
...
function forums_non_supporter_output() {
	global $blog_id, $psts;
	?>
	<h3><?php _e('Nur für Pro...', 'psforum'); ?></h3>
...
function forums_admin_bar( $menu ) {
	unset( $menu['admin.php?page=psforum'] );
	return $menu;
}
		...
		if ($tmp_forum_count > 0){
			...
			if ($_action == 'new_topic'){
				...
				if ($tmp_user_count > 0){
					...
					$tmp_errors = $tmp_errors + 1;
					$tmp_error_msg = __( 'Ungültiges Benutzerkonto...', 'psforum' );
				}
				//check for invalid forum ID
				if ($_POST['fid'] != $tmp_fid){
					$tmp_errors = $tmp_errors + 1;
					$tmp_error_msg = __( 'Ungültiges Forum...', 'psforum' );
				}
				if ($tmp_errors > 0){
					...
					if ($tmp_tid == ''){
						...
						$content = $content . '<p><center>' . __( 'Thema hinzugefügt!', 'psforum' ) . '</center></p>';
				...
				if ($tmp_topic_count > 0){
					if ($_action == 'close_topic'){
						if(current_user_can('manage_options')) {
							...
							$tmp_msg = __( 'Thema geschlossen!', 'psforum' );
						} else {
							$tmp_msg = __( 'Zugriff verweigert...', 'psforum' );
						}
					}
					if ($_action == 'open_topic'){
						if(current_user_can('manage_options')) {
							...
							$tmp_msg = __( 'Thema eröffnet!', 'psforum' );
						} else {
							$tmp_msg = __( 'Zugriff verweigert...', 'psforum' );
						}
					}
					...
					if ($tmp_topic_closed == 1){
						$content = $content . '<h3>' . $tmp_topic_title . ' (' . __( 'Geschlossen', 'psforum' ) . ')</h3>';
					} else {
				...
				if ($tmp_topic_count > 0){
					if(current_user_can('manage_options')) {
						...
						$content = $content . '<p><center>' . __( 'Zugriff verweigert...', 'psforum' ) . '</center></p>';
					}
				} else {
						$content = $content . '<p><center>' . __( 'Zugriff verweigert...', 'psforum' ) . '</center></p>';
				}
				...
				if ($tmp_topic_count > 0){
					if ( isset($_POST['Cancel']) ) {
						...
						if(current_user_can('manage_options')) {
							...
							if ($tmp_errors > 0){
								...
								$content = $content . '<p><center>' . __( 'Fehler beim Löschen des Themas...', 'psforum' ) . '</center></p>';
								$content = $content . '<br />';
								$content .= forums_get_metadata( $tmp_topic_last_updated, $tmp_topic_last_author );
								$content .= forums_get_navigation( $post->ID, (int)$_GET['topic'], $tmp_topic_closed );
								$content = $content . forums_output_topic_nav((int)$_POST['tid']);
								...
								forums_forum_count_posts($tmp_fid);
								forums_forum_count_topics($tmp_fid);
								$content = $content . '<p><center>' . __( 'Thema gelöscht!', 'psforum' ) . '</center></p>';
								$content = $content . forums_output_search_form($tmp_fid);
								$content = $content . '<br />';
								$content = $content . forums_output_forum_nav($tmp_fid);
								$content = $content . '<br />';
					...
					$content = $content . '<p><center>' . __( 'Ungültiges Thema!', 'psforum' ) . '</center></p>';
				}
				...
				if ( isset($_POST['Cancel']) ) {
					...
					if ($tmp_errors > 0){
						echo '<script type="text/javascript">';
						echo 'window.location="' . add_query_arg( array( 'topic' => (int)$_POST['tid'], 'forum_page' => (int)$_POST['forum_page'], 'msg' => urlencode( __('Fehler beim Löschen des Beitrags', 'psforum' ) ) ), get_permalink() ) . '#post-' . (int)$_POST['pid'] . '";';
						echo '</script>';
					} else {
						echo '<script type="text/javascript">';
						echo 'window.location="' . add_query_arg( array( 'topic' => (int)$_POST['tid'], 'msg' => urlencode( __( 'Beitrag gelöscht', 'psforum' ) ) ), get_permalink() ) . '";';
						echo '</script>';
					}
				}
				exit();
				...
				if ( isset($_POST['Cancel']) ) {
					...
					if ($_POST['post_content'] == ''){
						...
						if(current_user_can('manage_options')) {
							...
							echo '<script type="text/javascript">';
							echo 'window.location="' . add_query_arg( array( 'topic' => (int)$_POST['tid'], 'forum_page' => (int)$_POST['forum_page'], 'msg' => urlencode( __( 'Beitrag aktualisiert...', 'psforum' ) ) ), get_permalink() ) . '";';
							echo '</script>';
							exit();
						} else {
				...
				if ($user_ID == '' || $user_ID == '0'){
					$content = $content . '<p><center>' . sprintf( __( 'Du musst ein registrierter und angemeldeter Benutzer dieses Blogs sein, um in diesem Forum posten zu können. Bitte <a href="%s">anmelden</a> oder %s.', 'psforum' ), wp_login_url(),  wp_register('', '', false) ) . '</center></p>';
				} else {
					...
					if ($tmp_topic_closed != '1'){
						if ($_POST['post_content'] == ''){
							...
							echo '<script type="text/javascript">';
							echo 'window.location="' . add_query_arg( array( 'topic' => (int)$_POST['tid'], 'forum_page' => $tmp_total_pages, 'msg' => urlencode( __( 'Beitrag hinzugefügt...', 'psforum' ) ) ), get_permalink() ) . '";';
							echo '</script>';
							exit();
						}
					}
			...
			$content = __( 'Ungültiger Forum-Code', 'psforum' );
		}
		//insert/post content
...
function forums_get_navigation( $post_id, $topic_id, $topic_closed ) {
	...
	$content .= '<a href="' . get_permalink() . '">' . __( 'Zurück zum Index', 'psforum' ) . '</a> | ';
	if(current_user_can('manage_options')) {
		$content .= '<a href="' . add_query_arg( array('action' => 'delete_topic', 'tid' => $topic_id ), get_permalink() ) . '">' . __( 'Thema löschen', 'psforum' ) . '</a> | ';
		if ($topic_closed == 1){
			$content .= '<a href="' . add_query_arg( array('action' => 'open_topic', 'topic' => $topic_id ), get_permalink() ) . '">' . __( 'Thema öffnen', 'psforum' ) . '</a>';
		} else {
			$content .= '<a href="' . add_query_arg( array('action' => 'close_topic', 'topic' => $topic_id ), get_permalink() ) . '">' . __( 'Thema schließen', 'psforum' ) . '</a>';
		}
	}
	$content .= '</center>';
	$content .= '<hr />';
...
function forums_get_metadata( $last_updated, $last_author ) {
	...
	$content .= '<li>' . __( 'Letzte Aktualisierung: ', 'psforum' ) . date(get_option('date_format', __("D, F jS Y g:i A", 'psforum')),$last_updated) . '</li>';
	$content .= '<li>' . __( 'Letzte Antwort von: ', 'psforum' ) . forums_author_display_name($last_author) . '</li>';
	$content .= '</ul>';
	return $content;
}
...
function forums_output_search_results($tmp_fid,$tmp_query){
	...
	$content = $content . '<h3>' . __( 'Suchergebnisse', 'psforum' ) . '</h3>';
	$content = $content . forums_output_search_form($tmp_fid);
	$content = $content . '<br />';
	$content = $content . '<h3>' . __( 'Beiträge:', 'psforum' ) . '</h3>';
	...
	if (count($tmp_results) > 0){
		...
		$content =  $content . '<p><center>' . __( 'Keine Treffer...', 'psforum' ) . '</center></p>';
	}
...
function forums_output_search_form($tmp_fid){
	...
	if ($tmp_topic_count > 0){
		...
		$content = $content . '<input type="submit" name="Submit" value="' . __( 'Suche', 'psforum' ) . ' &raquo;" />';
...
function forums_output_new_post_separate($tmp_fid,$tmp_tid,$tmp_errors,$tmp_error_msg = ''){
	...
	if ($user_ID == '' || $user_ID == '0'){
		...
		if ($tmp_topic_closed != '1'){
			$content = $content . '<h3>' . __( 'Neuer Beitrag', 'psforum' ) . '</h3>';
			if ($tmp_errors > 0){
				if ($tmp_error_msg == ''){
					$tmp_error_msg = __( 'Du musst alle erforderlichen Felder ausfüllen...', 'psforum' );
				}
			...
			$content = $content . '<input type="submit" name="Submit" value="' . __( 'Beitrag senden &raquo;', 'psforum' ) . '" />';
			$content = $content . '</p>';
			$content = $content . '</form>';
		}
	}
...
function forums_output_new_post($tmp_fid,$tmp_tid,$tmp_errors,$tmp_error_msg = '') {
	...
	if ($user_ID == '' || $user_ID == '0'){
		...
		if ($tmp_topic_closed != '1'){
			...
			if ($tmp_errors > 0){
				if ($tmp_error_msg == ''){
					$tmp_error_msg = __( 'Du musst alle erforderlichen Felder ausfüllen...' );
				}
			...
			$content = $content . '</fieldset>';
			$content = $content . '<p class="submit">';
			$content = $content . '<input type="submit" name="Submit" value="' . __( 'Beitrag senden', 'psforum' ) . ' &raquo;" />';
			$content = $content . '</p>';
			$content = $content . '</form>';
		}
	}
...
function forums_output_edit_post($tmp_pid,$tmp_fid,$tmp_tid,$tmp_errors,$tmp_error_msg = '',$tmp_page = '') {
	...
	$content = $content . '<h3>' . __( 'Beitrag bearbeiten', 'psforum' ) . '</h3>';
	if ($tmp_errors > 0){
		if ($tmp_error_msg == ''){
			$tmp_error_msg = __( 'Du musst alle erforderlichen Felder ausfüllen...', 'psforum' );
		}
	...
	$content = $content . '</fieldset>';
	$content = $content . '<p class="submit">';
	$content = $content . '<input type="submit" name="Submit" value="' . __( 'Aktualisieren &raquo;', 'psforum' ) . '" />';
	$content = $content . '<input type="submit" name="Cancel" value="' . __( 'Abbrechen &raquo;', 'psforum' ) . '" />';
	$content = $content . '</p>';
	$content = $content . '</form>';
	return $content;
}
...
function forums_output_delete_post($tmp_pid,$tmp_tid,$tmp_page) {
	...
	if ($user_ID == '' || $user_ID == '0'){
		$content = $content . '<h3>' . __( 'Beitrag löschen', 'psforum' ) . '</h3>';
		$content = $content . '<p><center>' . __( 'Du musst eingeloggt sein...', 'psforum' ) . '</center></p>';
	} else {
		$content = $content . '<h3>' . __( 'Beitrag löschen', 'psforum' ) . '</h3>';
		$content = $content . '<br />';
		$content = $content . '<p>' . __( 'Möchtest Du diesen Beitrag wirklich löschen?', 'psforum' ) . '</p>';
		...
		$content = $content . '<p class="submit">';
		$content = $content . '<input type="submit" name="Submit" value="' . __( 'Löschen &raquo;', 'psforum' ) . '" />';
		$content = $content . '<input type="submit" name="Cancel" value="' . __( 'Abbrechen &raquo;', 'psforum' ) . '" />';
		$content = $content . '</p>';
		$content = $content . '</form>';
	}
...
function forums_output_delete_topic($tmp_fid,$tmp_tid) {
	...
	if ($user_ID == '' || $user_ID == '0'){
		$content = $content . '<h3>' . __( 'Thema löschen', 'psforum' ) . '</h3>';
		$content = $content . '<p><center>' . __( 'Du musst eingeloggt sein...', 'psforum' ) . '</center></p>';
	} else {
		$content = $content . '<h3>' . __( 'Thema löschen', 'psforum' ) . '</h3>';
		$content = $content . '<br />';
		$content = $content . '<p>' . __( 'Möchtest Du dieses Thema wirklich löschen?', 'psforum' ) . '</p>';
...
function forums_output_topic_nav($tmp_tid){
	...
	if ($tmp_current_page == 1){
		...
		$content = $content . '<td width="25%" style="text-align:left"><a href="'. add_query_arg( array( 'topic' => (int)$_GET['topic'], 'forum_page' => $tmp_previus_page ), get_permalink() ) . '">' . __(' &laquo; Bisherige') . '</a></td>';
	}
	$content = $content . '<td><center>' . sprintf( __( 'Zeige %1s > %2s von %3s Beiträgen', 'psforum' ), $tmp_showing_low, $tmp_showing_high, $tmp_post_count ) . '</center></td>';
	if ($tmp_current_page == $tmp_total_pages){
		//last page
		$content = $content . '<td width="25%" style="text-align:right"></td>';
	} else {
		$tmp_next_page = $tmp_current_page + 1;
		$content = $content . '<td width="25%" style="text-align:right"><a href="'. add_query_arg( array( 'topic' => (int)$_GET['topic'], 'forum_page' => $tmp_next_page ), get_permalink() ) . '">' . __( 'Weitere  &raquo;', 'psforum' ) . '</a></td>';
	}
	$content = $content . '</tr>';
	$content = $content . '</table>';
	return $content;
...
function forums_output_forum_nav($tmp_fid){
	...
	if ($tmp_topic_count > 0){
		...
		if ($tmp_current_page == 1){
			...
			$tmp_previus_page = $tmp_current_page - 1;
			$content = $content . '<td width="25%" style="text-align:left"><a href="'. add_query_arg( array( 'forum_page' => $tmp_previus_page ), get_permalink() ) . '">&laquo; ' . __( 'Bisherige', 'psforum' ) . '</a></td>';
		}
		$content = $content . '<td ><center>' . sprintf( __( 'Zeige %1s > %2s von %3s Beiträgen', 'psforum' ), $tmp_showing_low, $tmp_showing_high, $tmp_topic_count ) . '</center></td>';
		if ($tmp_current_page == $tmp_total_pages){
			//last page
			$content = $content . '<td width="25%" style="text-align:right"></td>';
		} else {
			$tmp_next_page = $tmp_current_page + 1;
			$content = $content . '<td width="25%" style="text-align:right"><a href="'. add_query_arg( array( 'forum_page' => $tmp_next_page ), get_permalink() ) . '">' . __( 'Weitere', 'psforum' ) . ' &raquo;</a></td>';
		}
		$content = $content . '</tr>';
		$content = $content . '</table>';
	}
...
function forums_output_view_topic($tmp_tid,$tmp_fid){
	...
	if (count($tmp_posts) > 0){
		...
		foreach ($tmp_posts as $tmp_post){
			...
			$content =  $content . __( 'Veröffentlicht: ', 'psforum' ) . date(get_option('date_format', __("D, F jS Y g:i A", 'psforum' )),$tmp_post['post_stamp']);
			...
			if(current_user_can('manage_options')){
				$tmp_post_count = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM " . $db_prefix . "forums_posts WHERE post_topic_ID = %d", $tmp_tid) );
				if ($tmp_post_count > 1){
					$content =  $content . '<a href="' . add_query_arg( array( 'action' => 'edit_post', 'forum_page' => $tmp_current_page, 'tid' => $tmp_tid, 'pid' => (int)$tmp_post['post_ID'] ), get_permalink() ) . '">' . __( 'Bearbeiten', 'psforum' ) . '</a>|<a href="?action=delete_post&page=' . $tmp_current_page . '&tid=' . $tmp_tid . '&pid=' . (int)$tmp_post['post_ID'] . '">' . __( 'Löschen', 'psforum' ) . '</a>';
				} else {
					$content =  $content . '<a href="' . add_query_arg( array( 'action' => 'edit_post', 'forum_page' => $tmp_current_page, 'tid' => $tmp_tid, 'pid' => (int)$tmp_post['post_ID'] ), get_permalink() ) . '">' . __( 'Bearbeiten', 'psforum' ) . '</a>';
				}
		...
		$content =  $content . '<table border="0" width="100%" cellpadding="0" cellspacing="0">';
		$content =  $content . '<tr>';
		$content =  $content . '<td><center>' . __( 'Keine Beiträge zum Anzeigen...', 'psforum' ) . '</center></td>';
		$content =  $content . '</tr>';
		$content =  $content . '</table>';
	}
...
function forums_output_new_topic($tmp_fid, $tmp_errors,$tmp_error_msg = '') {
	...
	if ($user_ID == '' || $user_ID == '0'){
		$content = $content . '<h3>' . __( 'Neues Thema', 'psforum' ) . '</h3>';
		$content = $content . '<p><center>' . __( 'Du musst eingeloggt sein...', 'psforum' ) . '</center></p>';
	} else {
		$content = $content . '<h3>' . __( 'Neues Thema', 'psforum' ) . '</h3>';
		if ($tmp_errors > 0){
	...
	if ($tmp_topic_count > 0){
		if ($user_ID == '' || $user_ID == '0'){
			...
			<tr style="background-color:' . $tmp_forum_color_header . ';">
				<th ' . $style . ' ><center>' . __( 'THEMEN', 'psforum' ) . '</center></th>
				<th ' . $style . ' ><center>' . __( 'BEITRÄGE', 'psforum' ) . '</center></th>
				<th ' . $style . ' ><center>' . __( 'NEUESTER BEITRAG', 'psforum' ) . '</center></th>
			</tr>';
		} else {
			$content = '<table ' . $style . ' width="100%" cellpadding="0" cellspacing="0">
			<tr style="background-color:' . $tmp_forum_color_header . ';">
				<th ' . $style . ' ><center>' . __( 'THEMEN', 'psforum' ) . ' (<a href="' . add_query_arg( array( 'action' => 'new_topic', 'fid' => $tmp_fid ), get_permalink() ) . '">' . __( 'NEUES THEMA', 'psforum' ) . '</a>)</center></th>
				<th ' . $style . ' ><center>' . __( 'BEITRÄGE', 'psforum' ) . '</center></th>
				<th ' . $style . ' ><center>' . __( 'NEUESTER BEITRAG', 'psforum' ) . '</center></th>
			</tr>';
	...
	if (count($tmp_topics) > 0){
		...
		//=========================================================//
			...
			if ($tmp_topic['topic_closed'] == 1){
				$content =  $content . '<td ' . $style . ' ><center><a href="'. add_query_arg( array( 'topic' => (int)$tmp_topic['topic_ID'] ), get_permalink() ) . '">' . esc_html(stripslashes($tmp_topic['topic_title'])) . ' (' . __( 'Geschlossen', 'psforum' ) . ')</a></center></td>';
			} else {
		...
		if ($user_ID == '' || $user_ID == '0'){
			$content =  $content . '<table border="0" width="100%" cellpadding="0" cellspacing="0">';
			$content =  $content . '<tr>';
			$content =  $content . '<td><center>' . __( 'Keine Themen zum Anzeigen...', 'psforum' ) . '</center></td>';
			$content =  $content . '</tr>';
			$content =  $content . '</table>';
		} else {
			$content =  $content . '<table border="0" width="100%" cellpadding="0" cellspacing="0">';
...
function forums_manage_options_output() {
	...
	if(!current_user_can($perms)) {
		echo "<p>" . __( 'Netter Versuch...', 'psforum' ) . "</p>";  //If accessed properly, this message doesn't appear.
		return;
	}
	echo '<div class="wrap">';
	...
	<form name="form1" method="post" action="<?php echo $page; ?>?page=psource_forum_settings&action=update_settings">
		<table class="form-table">
			...
			<tr valign="top">
				<th scope="row"><?php _e( 'Beiträge pro Seite', 'psforum' ) ?></th>
			...
			<tr valign="top">
				<th scope="row"><?php _e( 'Maximale Anzahl von Foren', 'psforum' ) ?></th>
			...
			<?php if (function_exists('is_pro_site')) { ?>
				<tr valign="top">
					<th scope="row"><?php _e( 'Maximale Anzahl von Foren für Pro Blogs', 'psforum' ) ?></th>
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Upgrades zulassen', 'psforum' ) ?></th>
...
function forums_manage_output() {
	...
	switch( $action ) {
		...
		$tmp_forums_count = $wpdb->get_var( $wpdb->prepare("SELECT COUNT(*) FROM " . $db_prefix . "forums WHERE forum_blog_ID = %d", $wpdb->blogid) );
			...
			?>
            <h2><?php _e( 'Foren verwalten', 'psforum' ) ?> (<a href="admin.php?page=psforum&action=new_forum"><?php _e( 'Neues Forum', 'psforum' ) ?></a>)</h2>
            <?php
			} else {
			?>
            <h2><?php _e( 'Foren verwalten', 'psforum' ) ?></h2>
            <?php
			...
			if ($forums_enable_upgrades && $tmp_forums_count == 0){
				if ( function_exists('is_pro_site') && !is_pro_site() && function_exists('psts_hide_ads') && !psts_hide_ads() ) {
					global $psts, $blog_id;
					$feature_message = __( "Aktualisiere Deinen Blog jetzt, um Foren hinzuzufügen, den Speicherplatz zu vergrößern, Videos einzubetten und mehr.", 'psforum' );
			...
			if (count($tmp_forums) > 0){
				...
				echo "<td valign='top'><a href='admin.php?page=psforum&action=edit_forum&fid=" . (int)$tmp_forum['forum_ID'] . "' rel='permalink' class='edit'>" . __( 'Bearbeiten', 'psforum' ) . "</a></td>";
				echo "<td valign='top'><a href='admin.php?page=psforum&action=delete_forum&fid=" . (int)$tmp_forum['forum_ID'] . "' rel='permalink' class='delete'>" . __( 'Entfernen', 'psforum' ) . "</a></td>";
				echo "</tr>";
				$class = ('alternate' == $class) ? '' : 'alternate';
				//=========================================================//
				}
		...
		case "new_forum":
			...
			if ($tmp_forums_count < $forums_max_forums){
				...
				<form name="form1" method="POST" action="admin.php?page=psforum&action=new_forum_process">
					...
					<br />
					<?php _e( 'Optional', 'psforum' ) ?></td>
					</tr>
					<tr valign="top">
					<th scope="row"><?php _e( 'Hauptfarbe', 'psforum' ) ?></th>
					...
					<br />
					<?php _e( 'Optional - Beispiel: #000000 ODER #FFFFFF', 'psforum' ) ?></td>
					</tr>
					<tr valign="top">
					<th scope="row"><?php _e( 'Sekundäre Farbe', 'psforum' ) ?></th>
					...
					<tr valign="top">
					<th scope="row"><?php _e( 'Header Farbe', 'psforum' ) ?></th>
					...
					<tr valign="top">
					<th scope="row"><?php _e( 'Rahmen Farbe', 'psforum' ) ?></th>
					...
					<tr valign="top">
					<th scope="row"><?php _e( 'Rahmengröße', 'psforum' ) ?></th>
		...
		case "new_forum_process":
			if ($_POST['forum_name'] == ''){
				...
				window.location='admin.php?page=psforum&updated=true&updatedmsg=" . urlencode( __( 'Forum hinzugefügt.', 'psforum' ) ) . "';
				</script>
				";
				exit();
			}
		...
		<br />
            ...
            <p class="submit">
            <input type="submit" name="Submit" value="<?php _e( 'Änderungen speichern', 'psforum' ) ?>" />
            </p>
            </form>
        <?php
		break;
		...
		case "edit_forum_process":
			...
			if ($tmp_forum_count > 0){
			    ...
			    <br />
					...
					echo "
					<SCRIPT LANGUAGE='JavaScript'>
					window.location='admin.php?page=psforum&updated=true&updatedmsg=" . urlencode( __( 'Einstellungen gespeichert.', 'psforum' ) ) . "';
					</script>
					";
					exit();
				}
		...
		case "delete_forum":
		?>
				<h2><?php _e( 'Forum entfernen', 'psforum' ) ?></h2>
                <p><?php _e( 'Möchtest Du dieses Forum wirklich entfernen? Alle Themen und Beiträge werden gelöscht.', 'psforum' ) ?></p>
				<form name="step_one" method="POST" action="admin.php?page=psforum&action=delete_forum_process">
				<input type="hidden" name="fid" value="<?php echo (int)$_GET['fid']; ?>" />
				<p class="submit">
				<input type="submit" name="Submit" value="<?php _e( 'Fortsetzen', 'psforum' ) ?>" />
				<input type="submit" name="Cancel" value="<?php _e( 'Abbrechen', 'psforum' ) ?>" />
				</p>
				</form>
        <?php
		break;
		...
		case "delete_forum_process":
			if ( isset($_POST['Cancel']) ) {
				...
				echo "
				<SCRIPT LANGUAGE='JavaScript'>
				window.location='admin.php?page=psforum&updated=true&updatedmsg=" . urlencode( __( 'Forum entfernt.', 'psforum' ) ) . "';
				</script>
				";
			}
			exit();
